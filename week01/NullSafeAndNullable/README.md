...
## C#におけるnull安全
- C#では値型(`int`,`bool`など)は通常nullを取ることができない。
- nullを許容するには、Nullable型を使用する。
	- Nullable型は値型に`?`を付けることで定義できる。
	- 例: `int?`, `bool?`

```csharp
int? number = null; // ok(nullを代入できる)
int number2 = null; // エラー(nullを代入できない)
```
- `T?`は`Nullable<T>`の省略記法。
- 参照型(`string`,`object`など)はC# 8以降で「nullable refarence types」もサポート。
